...
export default class SeguimientoMaltratoComponent implements OnInit {
  ...
  limpiarArchivo(input?: HTMLInputElement): void {
    this.selectedFile = null;
  ...
  enviarSeguimiento(): void {
    ...
    formData.append(
      'estadoInvestigacion',
      this.seguimientoForm.value.nuevoEstado,
    );
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          const mensaje = Array.isArray(backendMessage)
            ? backendMessage.join(' ')
            : backendMessage || 'Error al registrar el seguimiento.';
